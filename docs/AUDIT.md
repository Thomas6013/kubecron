...
| BUG-21 | 2026-08-21 | MED | storage | `ListJobRunsPaged` returned **zero rows for every cursored page**: the driver stores a `time.Time` in Go's `time.Time.String()` layout, which is not ISO 8601, so SQLite's `datetime()` yields NULL and `datetime(started_at) < datetime(?)` was NULL for every row. Silent — the UI's "Load more" simply stopped at 50 runs and run history appeared to end there | FIXED 2026-08-21 — cursor is a `time.Time`, compared directly (`queries.go`); relies on the same lexicographic property the `ORDER BY` on that column already did. Cursor also widened from whole seconds to full precision, which would otherwise skip parallel pods starting in the same second | `paging_test.go` (3 tests: full walk, zero cursor, same-second) |
| SEC-30 | 2026-08-21 | LOW | api | New `mode: server` surface: `/api/v1` and `/metrics` disclose the full CronJob inventory, run outcomes and captured **log bodies** to any caller that reaches the Service | MITIGATED 2026-08-21 — `API_TOKEN` bearer gate covers `/metrics` too (probes stay open for the kubelet); chart's `validateExposure` refuses an externally-reachable token-less install; startup `slog.Warn` when unset. Residual: anonymous is still the default for a ClusterIP install | `mode_test.go` (bearer matrix, `/metrics` gate, probe exemption) |
| BUG-22 | 2026-08-21 | MED | schedule | `PrevRun`'s fixed 25-hour lookback found no occurrence for any schedule sparser than daily, so `IsMissed` answered false for weekly/monthly CronJobs whether healthy or dead — missed detection was off for the badge, the `/api/v1` field and `kubecron_cronjob_missed` alike. Compounded by `IsMissed` answering false whenever the latest occurrence sat inside the grace period, which a per-minute schedule always does | FIXED 2026-08-21 — window derived from the schedule's cadence and expanded (also ~1 500 → ~3 `Next()` calls per per-minute row); grace-period step-back, guarded on `last != nil` so a newly-created CronJob is not flagged for a tick predating it. Found while porting to KubeDeck; both implementations diffed over 13 scenarios, 0 divergence | `missed_test.go` (4 new: sparse `PrevRun`, sparse `IsMissed`, on-the-hour, new-CronJob) |
| SEC-31 | 2026-08-21 | INFO | helm | Collector mode must not be able to mutate CronJobs — two components mutating the same object means two authorization models to keep in agreement, and KubeCron's is off by default | FIXED 2026-08-21 — enforced twice over: mutating routes are never registered (`server.go:registerUI`), and the chart grants no `patch`/`create` verb in `mode: server`, so the property is checkable from outside the process | `mode_test.go:TestServerModeRegistersNoMutatingRoute`, `helm template` matrix |
